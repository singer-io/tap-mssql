...
class LogBasedNoPkTest(BaseTapTest):
    ...
    def test_run(self):
        ...
        self.assertIn('Cannot sync stream: log_based_no_pk_dbo_int_data_no_pk using log-based replication', exit_status['tap_error_message'])
        self.assertIn('Change Tracking is not enabled for table: int_data_no_pk', exit_status['tap_error_message'])
